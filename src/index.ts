...
async function skillsCommand(): Promise<void> {
    const spinner = ora({ text: 'Loading skills...', stream: process.stderr }).start();
...
/**
 * Prompts the user for input via readline, returning a promise that
 * properly cleans up listeners on resolve, close, or error.
 */
...
async function interactiveMode(): Promise<void> {
    ...
    while (!shouldExit) {
        try {
            const input = await askQuestion(rl, chalk.cyan('You: '));
...
function showHelp(): void {
    ...
    console.log(chalk.white('  jarvis chat ""  Send a single message'));
